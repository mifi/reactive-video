...
export default function ReactiveVideo() {
  const { width, height, userData: { videoUri, title, description }, currentTime } = useVideo<UserData>();

  const blurAmount = Math.floor((width * 0.008) / (1 + currentTime));
  const brightnessAmount = Math.max(1, 1 + 5 * (1 - currentTime));
  const videoStyle: CSSProperties = { width, height, position: 'absolute', filter: `blur(${blurAmount.toFixed(5)}px) brightness(${brightnessAmount})` };
